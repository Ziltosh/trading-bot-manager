    ...
    RouterBuilder::<Shared>::new()
        ...
        .query("get_set_data", |t| {
            ...
            t(|_ctx, args: GetSetDataArgs| async move {
                ...
                let content = fs::read_to_string(&args.path)
                    .map_err(|err| -> RspcError { RspcError::new(BadRequest, err.to_string()) })?;
        ...
        .query("get_xlsm_basic_data", |t| {
            ...
            t(|_ctx, args: GetBasicDataArgs| async move {
                ...
                if workbook.is_err() {
                    return Err(RspcError::new(
                        BadRequest,
                        "Cannot open workbook".to_string(),
                    ));
                }
                ...
                if range.is_err() {
                    return Err(RspcError::new(
                        BadRequest,
                        "Cannot find worksheet 'Demarrage'".to_string(),
                    ));
                }
        ...
        .query("get_xlsm_lancement_data", |t| {
            ...
            t(|_ctx, args: GetLancementDataArgs| async move {
                ...
                let pct_periodes_rentables_b23 =
                    range.get((22, 1)).expect("Cannot get value at 22, 1");
                let resultat_total_b24 = range.get((23, 1)).expect("Cannot get value at 23, 1");
                let periode_resultat_moyen_b25 =
                    range.get((24, 1)).expect("Cannot get value at 24, 1");
                let periode_meilleur_resultat_b26 =
                    range.get((25, 1)).expect("Cannot get value at 25, 1");
                let periode_pire_resultat_b27 =
                    range.get((26, 1)).expect("Cannot get value at 26, 1");
                let dd_max_b28 = range.get((27, 1)).expect("Cannot get value at 27, 1");
                let check_periode_validation_debut_b30 =
                    range.get((29, 1)).expect("Cannot get value at 28, 1");
                let check_periode_validation_fin_b31 =
                    range.get((30, 1)).expect("Cannot get value at 30, 1");
                ...
                let check_resultat_mensuel_b34 =
                    range.get((33, 1)).expect("Cannot get value at 33, 1");
                ...
                let data = XlsmLancementData {
                    nb_periodes: nb_periodes_b22.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B22".to_string())
                    })?,
                    pct_periodes_rentables: pct_periodes_rentables_b23.as_string().ok_or_else(
                        || RspcError::new(BadRequest, "Cannot get value B23".to_string()),
                    )?,
                    resultat_total: resultat_total_b24.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B24".to_string())
                    })?,
                    periode_resultat_moyen: periode_resultat_moyen_b25.as_f64().ok_or_else(
                        || RspcError::new(BadRequest, "Cannot get value B25".to_string()),
                    )?,
                    periode_meilleur_resultat: periode_meilleur_resultat_b26.as_f64().ok_or_else(
                        || RspcError::new(BadRequest, "Cannot get value B26".to_string()),
                    )?,
                    periode_pire_resultat: periode_pire_resultat_b27.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B27".to_string())
                    })?,
                    dd_max: dd_max_b28.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B28".to_string())
                    })?,
                    check_periode_validation_debut: check_periode_validation_debut_b30
                        .as_datetime()
                        .ok_or_else(|| {
                            RspcError::new(BadRequest, "Cannot get value B30".to_string())
                        })
                        .unwrap()
                        .to_string(),
                    check_periode_validation_fin: check_periode_validation_fin_b31
                        .as_datetime()
                        .ok_or_else(|| {
                            RspcError::new(BadRequest, "Cannot get value B31".to_string())
                        })
                        .unwrap()
                        .to_string(),
                    check_passage: check_passage_b32.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B32".to_string())
                    })?,
                    check_resultat: check_resultat_b33.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B33".to_string())
                    })?,
                    check_resultat_mensuel: check_resultat_mensuel_b34.as_string().ok_or_else(
                        || RspcError::new(BadRequest, "Cannot get value B34".to_string()),
                    )?,
                    check_dd: check_dd_b35.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B35".to_string())
                    })?,
                    check_trades: check_trades_b36.as_f64().ok_or_else(|| {
                        RspcError::new(BadRequest, "Cannot get value B36".to_string())
                    })?,
                };
        ...
        .query("get_xlsm_optimisation_data", |t| {
            ...
            t(|_ctx, args: GetOptimisationDataArgs| async move {
                ...
                if range.is_err() {
                    return Err(RspcError::new(
                        BadRequest,
                        "Cannot find worksheet 'Optimisation'".to_string(),
                    ));
                }
                let range = range.unwrap();
                ...
                for i in 0..args.nb_periodes {
                    let periode = range
                        .get(((13 + i).try_into().unwrap(), 0))
                        .ok_or_else(|| RspcError::new(BadRequest, "Cannot get value A".to_string()))
                        .unwrap();
                    let resultat = range
                        .get(((13 + i).try_into().unwrap(), 1))
                        .ok_or_else(|| RspcError::new(BadRequest, "Cannot get value B".to_string()))
                        .unwrap();
                    let drawdown = range
                        .get(((13 + i).try_into().unwrap(), 2))
                        .ok_or_else(|| RspcError::new(BadRequest, "Cannot get value C".to_string()))
                        .unwrap();
                    periodes.push(
                        periode
                            .as_datetime()
        ...
        .query("get_xlsm_passage_data", |t| {
            ...
            t(|_ctx, args: GetPassageDataArgs| async move {
                ...
                if range.is_err() {
                    return Err(RspcError::new(
                        BadRequest,
                        "Cannot find worksheet 'AnalyseurPassages'".to_string(),
                    ));
                }
                let range = range.unwrap();
                ...
                while j < range.get_size().1 {
                    let parametre = range
                        .get((i, j))
                        .ok_or_else(|| RspcError::new(BadRequest, "Cannot get value".to_string()))
                        .unwrap();
                    parametres.push(
                        parametre
                            .as_string()
                            .ok_or_else(|| {
                                RspcError::new(BadRequest, "Cannot get value".to_string())
                            })?
                            .to_string(),
                    );
                    j += 1;
        ...
        .mutation("create", |t| {
            ...
            t(|ctx, args: OptimisationCreateArgs| async move {
                ...
                let new_set_path = format!(
                    "{}/SET_{}_{}_{}_{}.set",
                    &dir_set_path,
                    timeframe,
                    paire,
                    robot_id,
                    now.clone().unwrap().as_secs()
                );
                // println!("set path: {:?}", set_path);
                // println!("new set path: {:?}", new_set_path);
                fs::copy(set_path.clone(), new_set_path.clone())
                    .unwrap()
                    .to_string();
                ...
                fs::copy(xlsm_path.clone(), new_xlsm_path.clone())
                    .unwrap()
                    .to_string();
                ...
                if (compte_id == None) {
                    ...
                    optimisation = ctx
                        ...
                        .create(
                            ...
                            new_set_path,
                            new_xlsm_path,
                            vec![optimisation::compte::connect(compte::id::equals(
                                compte_id.unwrap(),
                            ))],
                        )
                        .exec()
        ...
        .mutation("delete", |t| {
            ...
            t(|ctx, args: OptimisationDeleteArgs| async move {
                let OptimisationDeleteArgs { id } = args;
